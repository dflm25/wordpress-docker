...
add_action( 'add_meta_boxes', function () {
	add_meta_box('portafolio_gallery', 'Media gallery', 'multi_media_uploader_meta_box_func', 'portafolio');
});

function multi_media_uploader_meta_box_func($post) {
	$banner_img = get_post_meta($post->ID, 'portafolio_post_banner_img', true);
?>
	
	<?php echo multi_media_uploader_field( 'portafolio_post_banner_img', $banner_img ); ?>
...
function multi_media_uploader_field($name, $value = '') {
	...
	if (!empty($value)) {
        ...
        <a href="' . $image_attributes[0] . '" target="_blank"><img src="' . $image_attributes[0] . '" />
        </a><i class="dashicons dashicons-no delete-img"></i>
        </li>';
			}
		}
	}
...
add_action( 'save_post', function ( $post_id ) {
	...
	if( isset( $_POST['portafolio_post_banner_img'] ) ){
		update_post_meta( $post_id, 'portafolio_post_banner_img', $_POST['portafolio_post_banner_img'] );
	}
});